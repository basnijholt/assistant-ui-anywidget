...
from .kernel_interface import KernelInterface, VariableInfo
...
class MessageHandlers:
    ...
    def _error_response(self, request_id: str, code: str, 
                       message: str, details: Optional[Any] = None) -> Dict[str, Any]:
    ...
    def handle_get_variables(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        max_preview_size = filter_params.get("max_preview_size", 100)
            ...
            if include_methods and var_info.is_callable:
                try:
                    ...
                    for attr in dir(value):
                        if not attr.startswith("_") and callable(getattr(value, attr, None)):
                            methods.append({
                                "name": attr,
                                "type": "method"
                            })
    ...
    def handle_execute_code(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        timeout = params.get("timeout")  # Not implemented yet
        ...
        if result.success:
            ...
            return self._error_response(
                ...
                result.error.get("message", "Execution failed") if result.error else "Unknown error",
                result.error
            )
    ...
    def handle_get_kernel_info(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        info.update({
            ...
            "execution_count": self.kernel.shell.execution_count if self.kernel.is_available and self.kernel.shell else 0,
    ...
    def handle_get_history(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if search:
            history = [
                h for h in history 
                if search.lower() in h.get("code", "").lower()
            ]
