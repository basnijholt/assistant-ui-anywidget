...
from assistant_ui_anywidget.kernel_interface import (
    KernelInterface, VariableInfo, ExecutionResult, StackFrame
)
...
class MockIPython:
    ...
    def _create_mock_array(self):
        ...
        mock_arr.__repr__ = Mock(return_value=f"array(shape=(10, 20), dtype=float64)")
        return mock_arr
...
def kernel_interface(mock_ipython):
    ...
    with patch("assistant_ui_anywidget.kernel_interface.get_ipython", return_value=mock_ipython):
...
class TestKernelInterfaceWithoutIPython:
    ...
    def test_without_ipython(self):
        ...
        with patch("assistant_ui_anywidget.kernel_interface.get_ipython", return_value=None):
            interface = KernelInterface()
