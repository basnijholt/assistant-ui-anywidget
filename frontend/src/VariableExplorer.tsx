  ...
  const filteredVariables = useMemo(() => {
    ...
    if (searchTerm) {
      ...
      filtered = filtered.filter(
        v => v.name.toLowerCase().includes(search) || v.type.toLowerCase().includes(search)
      );
    }
  ...
  return (
    ...
    >
            ...
            <tbody>
              {filteredVariables.map(variable => (
                ...
                >
                  ...
                  >
                    <span style={{ marginRight: "6px" }}>{getVariableIcon(variable.type)}</span>
                    <span style={{ fontFamily: "monospace" }}>{variable.name}</span>
      ...
      >
        ...
        {variables.length !== filteredVariables.length && ` (${variables.length} total)`}
      </div>
    </div>
  );
};
